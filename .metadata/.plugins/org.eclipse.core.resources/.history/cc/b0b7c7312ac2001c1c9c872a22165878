...
//	}
	...
	public String menu(Principal principal, Model model) throws Exception {
		...
		System.out.println("권한 체크" + principal.getName());
		if (principal != null) {
			JwtAuthenticationToken token = (JwtAuthenticationToken) principal;
			log.info("toString : " + token.getTokenAttributes().toString());
			log.info("role : " + token.getAuthorities());
			model.addAttribute("role", token.getAuthorities());
...
//	        throw new RuntimeException("failed");
	...
	@GetMapping("/token")
	public String token(Principal principal, Model model) {
		log.info("--------------------- main/token URL Page Move ------------------------------");
		JwtAuthenticationToken token = (JwtAuthenticationToken) principal;
		
		log.info("toString : " + token.getTokenAttributes().toString());
		log.info("role : " + token.getAuthorities());
		log.info("11111111"+token.getAuthorities().toString());
		String token222 = token.getAuthorities().toString();
		token222 = token222.substring(23);
		log.info("111"+token222);
		
		model.addAttribute("role", token.getAuthorities());
		model.addAttribute("token", principal.getName());
